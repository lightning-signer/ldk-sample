...
use lightning::chain::keysinterface::{DelayedPaymentOutputDescriptor, KeysInterface, Sign, SpendableOutputDescriptor, StaticPaymentOutputDescriptor, BaseSign, InMemorySigner};
...
impl Writeable for DynSigner {
    fn write<W: Writer>(&self, writer: &mut W) -> Result<(), ::std::io::Error> {
        ...
        // TODO(devrandom) make this polymorphic
        let inmem = inner.as_any()
            .downcast_ref::<InMemorySigner>().expect("only know about InMemorySigner for now");
        inmem.write(writer)
    }
}
